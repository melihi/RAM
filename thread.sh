...
curl -k   -s -o /dev/null -iL  --connect-timeout  60 -A "$useragent" -H "Profile: http://$1" -w "%{url} \n └──╼ Type: [%{content_type}] HTTP Code: [%{http_code}] Page Size: [%{size_download}] [Header]: Profile: $1 \n" "$1$2" &
curl -k   -s -o /dev/null -iL  --connect-timeout  60 -A "$useragent" -H "X-HTTP-DestinationURL: http://$1" -w "%{url} \n └──╼ Type: [%{content_type}] HTTP Code: [%{http_code}] Page Size: [%{size_download}] [Header]: X-HTTP-DestinationURL: $1 \n" "$1$2" &
